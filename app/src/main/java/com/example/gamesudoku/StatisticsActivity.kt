...
class StatisticsActivity : AppCompatActivity() {
    ...
    private fun showGameDetails(result: GameResult) {
        val dialogView = layoutInflater.inflate(R.layout.dialog_game_details, null)
        ...
        // Set the details
        dialogView.findViewById<TextView>(R.id.detailDate).text = dateFormat.format(result.date)
        dialogView.findViewById<TextView>(R.id.detailGame).text = "${result.boardSize}x${result.boardSize} • $difficultyText"
        dialogView.findViewById<TextView>(R.id.detailTime).text = timeText
        ...
        // Style the dialog with transparent background
        dialog.window?.setBackgroundDrawableResource(android.R.color.transparent)
        dialog.window?.setDimAmount(0.5f)
        dialog.show()
    }
