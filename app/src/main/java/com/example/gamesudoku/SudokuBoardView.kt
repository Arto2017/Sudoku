...
class SudokuBoardView(context: Context, attrs: AttributeSet) : View(context, attrs) {
    ...
    /**
     * Reveal hint for selected cell using proper hint algorithm
     * Finds valid hints based on current board state (not just solution board)
     * Checks for conflicts with incorrect user-entered values before applying hint
     */
    fun revealHint(): Boolean {
        ...
        // Check if solution board exists
        if (solutionBoard == null) {
            return false // No solution available
        }
        
        // FIRST: Check if there are ANY incorrect numbers (conflicts) on the board
        // Only check user-entered values (not fixed cells)
        if (hasIncorrectUserEnteredNumbers()) {
            lastHintErrorMessage = "Incorrect answers in board, fix first"
            return false // Block hint until all incorrect numbers are fixed
        }
        
        // Use hint algorithm to find a valid hint
        val hintResult = findValidHint(selectedRow, selectedCol)
        ...
        lastHintErrorMessage = null // Clear any previous error
        invalidate()
        return true
    }
